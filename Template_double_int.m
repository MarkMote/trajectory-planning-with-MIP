...
while search
    ...
    ax = l.define('ax',tau,N); 
    ay = l.define('ay',tau,N); 
    
    %% Initial and Final Conditions
    for n = 1:N
        l.declareAtom({[1,px(n,1)]  ,'=', pInit(1,n)} );
        l.declareAtom({[1,py(n,1)]  ,'=', pInit(2,n)} );
        l.declareAtom({[1,vx(n,1)]  ,'=', vInit(1,n)} );
        l.declareAtom({[1,vy(n,1)]  ,'=', vInit(2,n)} );
        
        l.declareAtom({[1,px(n,tau)]  ,'=', pFinal(1,n)});
        l.declareAtom({[1,py(n,tau)]  ,'=', pFinal(2,n)});
        l.declareAtom({[1,vx(n,tau)]  ,'=', vFinal(1,n)} );
        l.declareAtom({[1,vy(n,tau)]  ,'=', vFinal(2,n)} );
    end
    
    %% Dynamics
    for n = 1:N
        for i = 1:tau-1
            l.declareAtom({ [1, px(n, i+1), -1, px(n,i), -dt, vx(n,i), -0.5*dt^2, ax(n,i)], '=', 0});
            l.declareAtom({ [1, py(n, i+1), -1, py(n,i), -dt, vy(n,i), -0.5*dt^2, ay(n,i)], '=', 0});
            l.declareAtom({ [1, vx(n, i+1), -1, vx(n,i), -dt, ax(n,i)], '=', 0});
            l.declareAtom({ [1, vy(n, i+1), -1, vy(n,i), -dt, ay(n,i)], '=', 0});
        end
    end
    ...
    for n = 1:N
        for i = 1:tau
            ...
            l.normLeq([ax(n,i), ay(n,i)] , aMax , P); % ||[ax,ay||_2 < aMax approximated with P sided poly
        end
    end
